...
<div class="constraint">
    <div class="DivClass2">
	    <?php
		    ...
		    $sql = "select cname, time, place from seats where ID = \"" . $_SESSION["ID"] . "\" and valid = 1 order by cname desc, time desc";
			...
			echo "<tr><td>項次</td><td>課程名稱</td><td>預約時間</td><td>地點</td><td>刪除預約</td></tr>";
			while($row = mysqli_fetch_array($result) and $j <= $records_per_page){
				...
				echo "<td><a style=\"cursor: pointer;\" onclick=\"del('" . $_SESSION["ID"] . "'," . (string)($total) .")\">刪除</a></td></tr>";
				$total++;
				$j++;
			}
			echo "</table>";
			...
			if($pages > 1){
				echo "<a href='ModRes.php?Pages=" . ($pages-1) ."'>上一頁</a>| ";
			}
			for($i = 1; $i <= $total_pages; $i++){
				if($i != $pages){
					echo "<a href='ModRes.php?Pages=" . $i . "'>" . $i . "</a>";
				}
				else{
					echo $i . "";
				}	
			...
			if($pages < $total_pages){
				echo "|<a href='ModRes.php?Pages=" . ($pages+1) ."'>下一頁</a> ";
			}
